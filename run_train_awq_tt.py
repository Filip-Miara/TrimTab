...
D_INPUT = 3584; N_LAYERS = 28; BS = 64; LR = 3e-4; N_EPOCHS = 50
CHUNK_SIZE = 4000
GPU_CHUNK = 4200
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            r2_norm, mse_norm, cos = compute_metrics(v_pred_norm, val_v_norm_gpu)
            # Denormalized R² (original velocity scale)
            v_pred = v_pred_norm * v_std_gpu + v_mean_gpu
            r2, mse, _ = compute_metrics(v_pred, val_v_gpu)
        if r2 > best_r2:
            best_r2 = r2
            torch.save(tt.state_dict(), "best_awq_tt.pt")
            print(f"  ep={epoch+1:2d} loss={epoch_loss/max(n_batches,1):.6f} val_r²={r2:.4f} (norm={r2_norm:.4f}) cos={cos:.4f} \u2728 BEST {time.time()-t0:.0f}s", flush=True)
        else:
            print(f"  ep={epoch+1:2d} loss={epoch_loss/max(n_batches,1):.6f} val_r²={r2:.4f} (norm={r2_norm:.4f}) cos={cos:.4f} {time.time()-t0:.0f}s", flush=True)
