...
_knit_registry: dict[str, list] = {}


def _get_knit_stack(module_path: str) -> list:
    if module_path not in _knit_registry:
        _knit_registry[module_path] = []
    return _knit_registry[module_path]


def clear_knit_registry():
    _knit_registry.clear()


class KnitBVoRAN(LowRankAdapter):
