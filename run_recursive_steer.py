...
"""Test recursive steering: apply steering k times to the same layer."""
...
TARGET_LAYERS = [10]  # L10 best trim-tab
K_VALUES = [1, 2, 3, 4]
...
results = {"baseline": base_acc, "recursive": {}}

for k in K_VALUES:
    for li in TARGET_LAYERS:
        print(f"\nL{li} k={k} α={ALPHA}...", flush=True)
            ...
            for _ in range(MAX_GEN):
                ...
                if not first:
                    for recur in range(k):
                        ...
                        k_proj = ly.self_attn.k_proj(h_st.to(torch.bfloat16)).view(B, N_KV_HEADS, 1, HEAD_DIM)
                        v_proj = ly.self_attn.v_proj(h_st.to(torch.bfloat16)).view(B, N_KV_HEADS, 1, HEAD_DIM)
                        fwd.past_key_values.layers[li].keys[:,:,-1:,:] = k_proj.to(fwd.past_key_values.layers[li].keys.dtype)
                        fwd.past_key_values.layers[li].values[:,:,-1:,:] = v_proj.to(fwd.past_key_values.layers[li].values.dtype)
                        # Re-run forward pass with same token to get updated hidden states
                        if recur < k - 1:  # not the last iteration
                            with torch.no_grad():
        ...
        acc = sum(correct)/30; d = acc - base_acc
        key = f"L{li}_k{k}"
        results["recursive"][key] = acc
        print(f"  L{li} k={k}: {sum(correct)}/30 ({100*acc:.1f}%) Δ={100*d:+.1f}pp [{time.time()-t0:.0f}s]", flush=True)

print(f"\n{'='*60}")
print(f"RECURSIVE STEERING (α={ALPHA}, 30 problems)")
print(f"{'='*60}")
print(f"  Baseline: {100*base_acc:.1f}%")
for k in K_VALUES:
    for li in TARGET_LAYERS:
        a = results["recursive"][f"L{li}_k{k}"]; d = a - base_acc
        print(f"  L{li} k={k}: {100*a:5.1f}% ({100*d:+5.1f}pp)")

json.dump(results, open("recursive_steer_results.json", "w"), indent=2)
print(f"\nSaved to recursive_steer_results.json")
